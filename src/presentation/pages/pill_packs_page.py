...
def render_pill_packs_page(container: ApplicationContainer) -> None:
    ...
    repo = container.pill_pack_repository()
    sync_use_case = container.sync_pill_pack_executions_use_case()
    accuracy_use_case = container.calculate_pill_pack_accuracy_use_case()
    calc_time_use_case = container.calculate_processing_time_use_case()
    get_image_use_case = container.get_image_use_case()

    try:
        with open(container.config.pill_pack_answer_key_path(), "r", encoding="utf-8") as f:
        ...
        if selected_img_execution:
            ...
            if expected_data and selected_img_execution.result:
                try:
                    ...
                    individual_score = accuracy_use_case._evaluate_pill_pack(expected_data, predicted_data) * 100
